...
export class OutboxWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Fast-path: Adds a job to the Redis queue for immediate processing.
   */
  ...
  /**
   * Fallback: Scans for missed/failed events in PostgreSQL using SELECT ... FOR UPDATE SKIP LOCKED
   */
  public async reconcileOutbox(): Promise<void> {
    try {
      await this.outboxRepo.manager.transaction(async (transactionalEntityManager) => {
        const events = await transactionalEntityManager.find(OutboxOrmEntity, {
