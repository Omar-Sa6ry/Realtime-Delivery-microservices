...
export class SmsWorker extends WorkerHost {
  ...
  async process(job: Job<{ notificationId: string; deliveryId: string }>) {
    ...
    try {
      ...
      await this.notificationService.send(ChannelType.SMS as any, message);

      delivery.status = DeliveryChannelStatus.SENT;
      delivery.sentAt = new Date();
      await this.deliveryRepository.save(delivery);
      
      this.logger.debug(`SMS sent successfully for notification ${notificationId}`);
    } catch (error) {
      ...
      this.logger.error(`SMS failed for notification ${notificationId}: ${error.message}`);
      throw error;
    }
  }
}
