...
export class GrpcClient {
  ...
  async callDelivery(method: string, args: Record<string, unknown>): Promise<any> {
    ...
    if (!this.canAttempt(this.deliveryState, 'delivery')) return null;
    if (!this.delivery) {
      this.delivery = await this.createClient(
        'delivery',
        'DeliveryService',
  ...
  async callDriver(method: string, args: Record<string, unknown>): Promise<any> {
    ...
    if (!this.canAttempt(this.driverState, 'driver')) return null;
    if (!this.driver) {
      this.driver = await this.createClient(
        'driver',
        'DriverService',
    ...
    return this.invoke(this.driver, method, args, 'driver');
  }

  isDeliveryClientConfigured(): boolean {
    return !!this.deliveryService;
  }

  isDriverClientConfigured(): boolean {
